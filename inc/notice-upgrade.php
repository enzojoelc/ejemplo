...
class CoverNews_Notice
{
    ...
    {
        if (isset($_GET['covernews_upgrade_notice_dismiss']) && isset($_GET['_covernews_upgrade_notice_dismiss_nonce'])) { // WPCS: input var ok.
            ...
            $dismiss_notice = sanitize_text_field(wp_unslash($_GET['covernews_upgrade_notice_dismiss']));

            // Hide.
            if ($dismiss_notice === $_GET['covernews_upgrade_notice_dismiss']) {
    ...
    {
        if (isset($_GET['covernews_upgrade_notice_dismiss_temporary']) && isset($_GET['_covernews_upgrade_notice_dismiss_temporary_nonce'])) { // WPCS: input var ok.
            ...
            $dismiss_notice = sanitize_text_field(wp_unslash($_GET['covernews_upgrade_notice_dismiss_temporary']));

            // Hide.
            if ($dismiss_notice === $_GET['covernews_upgrade_notice_dismiss_temporary']) {
...
class CoverNews_Upgrade_Notice extends CoverNews_Notice {
    ...
    public function __construct() {
        ...
        $dismiss_url = wp_nonce_url(
            add_query_arg( 'covernews_upgrade_notice_dismiss', 'upgrade', admin_url() ),
            'covernews_upgrade_notice_dismiss_nonce',
            '_covernews_upgrade_notice_dismiss_nonce'
        );

        $temporary_dismiss_url = wp_nonce_url(
            add_query_arg( 'covernews_upgrade_notice_dismiss_temporary', 'upgrade', admin_url() ),
    ...
    private function set_temporary_dismiss_notice_time() {
        if ( isset( $_GET['covernews_upgrade_notice_dismiss_temporary'] ) && 'upgrade' === $_GET['covernews_upgrade_notice_dismiss_temporary'] ) {
    ...
    public function set_dismiss_notice() {
         ...
         */
        if ( get_option( 'covernews_upgrade_notice_start_time' ) > strtotime( '-2 minutes' )
        ...
        <div class="notice notice-success covernews-notice" >
            ...
            <p class="notice-text">
                ...
                /* Translators: %1$s current user display name., %2$s this theme name., %3$s discount coupon code., %4$s discount percentage. */
                   esc_html__(
                    ...
                    '<a href="https://afthemes.com/products/covernews/">CoverNews</a>',
            ...
            <div class="links">
                ...
                <a href="https://afthemes.com/supports/" class="button button-normal" target="_blank">
                    ...
                    <span><?php esc_html_e( 'Have Queries?', 'covernews' ); ?></span>
                </a>
            </div>
