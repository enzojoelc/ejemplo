...
add_action('wp_ajax_covernews_plugin_installer_activation', 'covernews_plugin_installer_activation'); // Activate plugin
...
function covernews_plugin_installer_activation()
{
  ...
  if (!empty($list_of_plugins)) {
    $data =  covernews_activate_plugin_list($sanitinzed_activate);
  }
...
function covernews_activate_plugin_list($sanitinzed_activate)
{
  $count = 0;
